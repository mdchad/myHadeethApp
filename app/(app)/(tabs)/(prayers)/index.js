...
export default function Prayer() {
  ...
  async function calculatePrayer(day, monthlyPrayerTimes) {
    ...
    ;['fajr', 'syuruk', 'dhuhr', 'asr', 'maghrib', 'isha'].forEach(
      (time, i) => {
        ...
        let elapsed = isBefore(
          new Date(getPrayerDate[time] * 1000),
          currentTime
        )

        const formattedPrayer = format(new Date(getWaktu * 1000), 'h:mm a', {
    ...
    if (!nextAvailablePrayer) {
      ...
      calculatePrayer(
        format(addDays(currentDate, 1), 'd', { timeZone: 'Asia/Kuala_Lumpur' }),
        monthlyPrayerTimes
      )
    }
  }
        ...
        >
          <View>
            ...
            <Text className="text-white text-xs">
              {formatHijri.format(calendarDate)}
            </Text>
          </View>
          <View className="flex items-end">
            ...
            <View className="flex flex-row">
              ...
              {userPlace && (
                <Text className="ml-1 text-white text-xs text-center">
                  {userPlace[0].city}
